...
class ImageCutter:
    ...
    def update_image_info(self):
        ...
        if self.image and self.original_image and self.image_path:
            ...
            if os.path.exists(self.image_path):
                ...
                self.file_size_var.set(self.format_file_size(file_size))
            else:
    ...
    def open_directory(self):
        ...
        if directory:
            ...
            self.image_files = [os.path.join(directory, f) for f in os.listdir(directory) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif'))]
            if self.image_files:
    ...
    def load_image(self, path):
        try:
            self.original_image = Image.open(path).convert("RGB")  # 统一格式，避免透明通道问题
            self.base_scale = 1.0  # 重置累计缩放因子
            self.image = self.original_image.copy()  # 初始化当前显示图像
    ...
    def scale_image(self, factor):
        ...
        # 计算新的累计缩放因子（保留3位小数，避免精度丢失）
        new_scale = round(self.base_scale * self.scale_step[factor], 3)
        # 限制缩放边界
        if new_scale < self.min_scale or new_scale > self.max_scale:
            print(f"缩放限制：已达{self.min_scale}x~{self.max_scale}x边界")
            return
        
        # 基于原始图片计算缩放后尺寸（避免多次缩放失真）
        orig_w, orig_h = self.original_image.size
        new_w = int(orig_w * new_scale)
        new_h = int(orig_h * new_scale)
        
        # 快速缩放：使用LANCZOS（高质量）+ 直接赋值，减少中间步骤
        self.image = self.original_image.resize((new_w, new_h), Image.LANCZOS)
    ...
    def cut_current_area(self):
        ...
        orig_x1 = max(0, int((x1 - self.image_x) / self.scale))
        orig_y1 = max(0, int((y1 - self.image_y) / self.scale))
        orig_x2 = min(self.image.width, int((x2 - self.image_x) / self.scale))
        orig_y2 = min(self.image.height, int((y2 - self.image_y) / self.scale))
        ...
        # 裁剪并保存
        cropped_img = self.image.crop((orig_x1, orig_y1, orig_x2, orig_y2))
        ...
        if self.image_path:
            ...
            name, ext = "cut", ".png"
        output_path = os.path.join(output_dir, f"{name}_scale{round(self.base_scale,2)}_cut_{orig_x1}_{orig_y1}{ext}")
        
        # 高质量保存
        cropped_img.save(output_path, quality=100, optimize=True)
